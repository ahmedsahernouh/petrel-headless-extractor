...
def main():
    ...
    with zipfile.ZipFile(args.zip) as archive:
        assert {Path(i.filename).parts[0] for i in archive.infolist()}=={'PetrelExtractor','run_portable_petrel_extract.bat'}
        assert [i.filename for i in archive.infolist() if i.filename.endswith('.bat')]==['run_portable_petrel_extract.bat']
        for entry in archive.infolist():
            assert (relocated/entry.filename).resolve().is_relative_to(relocated)
        archive.extractall(relocated)
    package=relocated/'PetrelExtractor';bat=relocated/'run_portable_petrel_extract.bat'
