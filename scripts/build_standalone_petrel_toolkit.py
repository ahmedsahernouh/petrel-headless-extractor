...
def main():
    ...
    (package/'STANDALONE.txt').write_text('Offline distribution. Keep the BAT, scripts and runtime together.\n',encoding='ascii')
    ...
    for wheel in sorted(wheels.glob('*.whl')):
        ...
        inventory.append({'name':meta['Name'],'version':meta['Version'],'wheel':wheel.name,
                          'sha256':digest(wheel),'license':meta.get('License-Expression') or meta.get('License','unspecified'),
