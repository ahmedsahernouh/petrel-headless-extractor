...
$report = [ordered]@{
    ...
    version = if (Test-Path -LiteralPath (Join-Path $toolkitRoot "toolkit.json")) { (Get-Content -LiteralPath (Join-Path $toolkitRoot "toolkit.json") -Raw | ConvertFrom-Json).version } else { "0.2.0" }
