...
| `FloatWellLog` / `IntWellLog` in native stores | LAS for scalar curves; CSV + metadata; an array format for image logs | **Not decoded.** Highest next parser priority. Must recover sample index, values, nulls, units, curve/well identity and categorical codes. Reading an existing LAS does not solve this. |
| Native surface arrays `.zhz` + `.zhz_msk`; `RegValGrid2` / `ValGrid2` | XYZ/CSV, ZMAP or GeoTIFF + domain/CRS | Development tiled-array decoder only. Historical demo run exported 10/12 files; 2 remained unresolved. Its survey transform uses a Petrel-authored SEG-Y donor and demo-specific node mapping. Generalize object/grid georeferencing before packaging. |
