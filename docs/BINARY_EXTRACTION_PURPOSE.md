...
| Native surface arrays `.zhz` + `.zhz_msk`; `RegValGrid2` / `ValGrid2` | XYZ/CSV, ZMAP for regular grids | v0.7.0 beta expands validated RegValGrid2 profiles and explicit ValGrid2 arrays. Regular grids export XYZ/CSV/ZMAP; explicit XYZ meshes export XYZ/CSV. Complete masks and report maps are retained; unknown units remain labelled. Other geometry and `.zhz` tile layouts remain unsupported. [Profile/evidence](NATIVE_LOGS_SURFACES.md). |
