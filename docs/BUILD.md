...
.venv\Scripts\python.exe scripts\build_standalone_petrel_toolkit.py --runtime-archive build\downloads\python-3.13.15-embed-amd64.zip --wheels build\downloads\wheels --output-root build\releases-v0.6.1
```

The builder verifies the official runtime SHA-256, installs only binary wheels matching `requirements-standalone-lock.txt` with `--require-hashes --no-index`, checks the embedded runtime, and writes `PetrelExtractor-0.6.1-win64.zip` plus a SHA-256 file. The ZIP contains one root BAT beside a `PetrelExtractor` support folder with `bootstrap/runtime.zip`; expanded `runtime/` files are omitted from the outer ZIP. On launch, Windows PowerShell validates the cache hash and installs only the managed runtime entries from the full installed-file inventory. The build staging folder retains the expanded runtime for validation. `zfpy` is pinned alongside the other dependencies, and preflight tests a numerical ZFP round-trip. A path-length gate models Explorer's additional ZIP-stem directory under Downloads. Use a fresh output directory for each build; existing package folders are not overwritten. Licenses and per-file inventories are included. Build timestamps and ZIP metadata vary; file hashes provide the exact identity of each release.

Run acceptance on the ZIP you built:

```powershell
.venv\Scripts\python.exe scripts\test_standalone_petrel_toolkit.py --zip "build\releases-v0.6.1\PetrelExtractor-0.6.1-win64.zip" --evidence-dir "build\acceptance-new"
```
