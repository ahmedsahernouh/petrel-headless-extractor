...
For unattended report-only runs, use `GeoViewer_data_extractor.bat "C:\Projects\Example.pet" "C:\PetrelOut" -ReportOnly -NoPause`. Report-only mode still preserves the native snapshot, inventories companions, and prepares supported native previews in disposable scratch space. It keeps figures, statistics, metadata and inventory tables; it discards temporary log/surface/spatial datasets. Those objects are labelled `preview_only`, never counted as exported datasets. Source links point to preserved binaries. The selected mode appears at the top of the report. The report works offline without a web server. Keep the whole extraction package together so its relative data links work after moving it.
...
- **Coverage charts:** decoded objects and a separate amber bar for numeric grid exports with unresolved units, against registry IDs, plus log/surface decoder outcomes. These are not a whole-project completion percentage.
...
- **Surface figures:** native grid-cell maps and full-data value distributions. A bounded selection of original grid nodes drives the map; cell colors average the four displayed corners. Any cell spanning an undefined native node or cell is masked. This preserves holes; thin features can be omitted by preview sampling. Units, domain/sign and unresolved CRS are explicit. No inferred fault connections or new triangulation. Older packages without grid previews retain their labelled XY point previews.
...
Default budgets: 64 log figures, 256 surface figures and 3 ZGY previews. Every discovered object remains in the data tree/catalogue even if no figure is generated. New grid previews retain at most 256 × 256 selected native nodes and full-grid statistics/histograms, so plotting does not re-read huge ASCII exports. Report-only mode produces these previews directly without writing grid ASCII datasets. Numeric CSV previews for logs and older packages remain bounded at 128 MiB and 2 million rows per object. Log plots display at most 2,500 points; old surface point previews at most 6,000. Each seismic patch reads at most 1 × 256 × 512 decoded samples; its statistics are patch-only.

CSV and grid-preview hashes must match the native decoder receipt before plotting and remain unchanged afterwards. Native log/surface figures require a completed/partial receipt with source preservation confirmed. Missing or changed artifacts produce a visible per-object preview issue. ZGY previews check file size/modification time around the read; they do not add a full-volume hashing pass. Existing extraction hashes remain separate provenance evidence.
